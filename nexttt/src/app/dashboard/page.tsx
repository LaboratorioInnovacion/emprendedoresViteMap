...
const Dashboard: React.FC = () => {
  ...
  const { emprendedores } = useEmprendedores(); // Contexto de emprendedores
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setNegocios(data);
        setFilteredBusinesses(data); // Por defecto, mostrar todos
      } catch (error) {
  ...
  const createBusinessIcon = (type: string) => {
    if (!L) return null; // Asegurarse de que Leaflet esté cargado
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Bienvenido al Sistema de Emprendedores</p>
        </div>
        ...
        <div className="lg:col-span-2 card">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              <Map size={20} className="text-primary-600 mr-2" />
              <h2 className="text-xl font-semibold">Mapa Negocios</h2>
            </div>
            <Link href="/map" className="text-sm text-primary-600 hover:underline flex items-center">

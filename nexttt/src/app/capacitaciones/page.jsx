              ...
              <table className="w-full rounded-lg text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                <thead className="bg-gray-100 dark:bg-gray-800">
                  <tr>
                    ...
                    <th className="text-gray-600 dark:text-white px-2 py-2">Tipo</th>
                    <th className="text-gray-600 dark:text-white px-2 py-2">Organismo</th>
                    <th className="text-gray-600 dark:text-white px-2 py-2">Modalidad</th>
                    <th className="text-gray-600 dark:text-white px-2 py-2">Descripción</th>
                    <th className="text-gray-600 dark:text-white px-2 py-2">Inicio</th>
                    <th className="text-gray-600 dark:text-white px-2 py-2">Fin</th>
                  ...
                  {capacitaciones.map((c, idx) => (
                    <tr key={c.id} className={`transition-colors ${idx % 2 === 0 ? 'bg-gray-50 dark:bg-gray-900' : 'bg-white dark:bg-gray-800'}`}>
                      ...
                      <td className="px-2 py-2">
                        <button
                          className="text-gray-700 dark:text-white text-base underline hover:text-primary-900 font-semibold"
                      ...
                      <td className="text-gray-600 dark:text-white px-2 py-2">{Array.isArray(c.tipo) ? c.tipo.join(', ') : c.tipo}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.organismo}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.modalidad ?? '-'}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.descripcion ?? '-'}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.fechaInicio ? new Date(c.fechaInicio).toLocaleDateString() : '-'}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.fechaFin ? new Date(c.fechaFin).toLocaleDateString() : '-'}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.cupo ?? '-'}</td>
                      <td className="text-gray-600 dark:text-white px-2 py-2">{c.observaciones ?? '-'}</td>

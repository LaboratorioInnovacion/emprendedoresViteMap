...
const Page = () => {
	...
	useEffect(() => {
		const fetchCapacitacion = async () => {
			try {
				...
				setForm({
					...data,
					tipo: Array.isArray(data.tipo) ? data.tipo.join(", ") : data.tipo || "",
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			router.push(`/capacitaciones/${params.id}`);
		} catch (err) {
			setError(err.message);
		} finally {
			setSaving(false);
	...
	return (
		<div className="max-w-2xl mx-auto p-4 animate-fadeIn">
			<div className="card shadow-lg border border-gray-200 dark:border-gray-700 p-6">
				<h1 className="text-2xl font-bold mb-6 text-primary-700 dark:text-primary-300">Editar Capacitación</h1>
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<div className="flex gap-2 justify-end mt-4">
						<button type="button" className="btn btn-secondary" onClick={() => router.push(`/capacitaciones/${params.id}`)}>Cancelar</button>

...
const EmprendedoresOtrosPage = () => {
  const router = useRouter();
  const { emprendedoresOtros, loading, error, fetchEmprendedoresOtros } = useEmpreOtros();
  ...
  useEffect(() => {
    fetchEmprendedoresOtros();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Filtrar por nombre/apellido, nivel de estudios y estado
  const filtered = emprendedoresOtros.filter((emp) => {
  ...
  // Definir viewport inicial para el mapa
  ...
  useEffect(() => {
    console.log("emprendedores otros cargados:", emprendedoresOtros);
  }, [emprendedoresOtros]);
    ...
    <div className="space-y-4">
      <div className="mb-4">
        <LeafletMapEmprendedores
          emprendedores={emprendedoresOtros.map((emp) => ({
            id: emp.id,
          ...
          onBusinessSelect={(id) => router.push(`/otros/${id}`)}
        />
      </div>
      {/* Header Section */}
...
export default EmprendedoresOtrosPage;
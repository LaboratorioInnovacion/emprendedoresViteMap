...
const BusinessList: React.FC<BusinessListProps> = ({ 
  ...
  const filteredBusinesses = businesses.filter(business => {
    ...
    if (filters.businessTypes.length > 0 && !filters.businessTypes.includes(business.type)) {
      return false;
    }
    
    if (filters.status.length > 0 && !filters.status.includes(business.status)) {
      return false;
    }
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {statusOptions.map(status => (
                  <button
                    key={status}
                    onClick={() => toggleStatusFilter(status as 'active' | 'inactive' | 'pending')}
                    className={`badge text-xs py-1.5 px-3 capitalize ${
                      filters.status.includes(status as 'active' | 'inactive' | 'pending')
                        ? getStatusBadgeClass(status as 'active' | 'inactive' | 'pending')
                        : 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300'
                    }`}
                  >
                    {status}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-base font-medium truncate">{business.name}</h3>
                      <span className="badge badge-secondary text-xs capitalize mt-1">{business.type}</span>
                    </div>
            ...
            {filteredBusinesses.length > 0 ? (
              filteredBusinesses.map(business => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400">{business.contact.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="badge badge-secondary capitalize">{business.type}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`badge ${getStatusBadgeClass(business.status)} flex items-center`}>
                      <Activity size={12} className="mr-1" />
                      {business.status.charAt(0).toUpperCase() + business.status.slice(1)}
                    </span>
                  </td>
